...
// [[Rcpp::export]]
double haver_dist(double long1,double lat1,double long2, double lat2){
  double out;
  double d_long = (long2 - long1)*M_PI/180.0;
  double d_lat = (lat2 - lat1)*M_PI/180.0;
  double a = pow(sin(d_lat/2.0),2.0) + cos(lat1*M_PI/180.0) * cos(lat2*M_PI/180.0) * pow(sin(d_long/2.0),2.0);
  out = 2 * atan2(sqrt(a),sqrt(1-a));
  return out;
}

NumericMatrix dist_mat_sphere(NumericVector lon,NumericVector lat){
  int n = lon.size();
  double d = 0;
  NumericMatrix out(n,n);
  ...
  for (int i = 0; i < n - 1; i++){
    for (int j = i + 1; j < n ; j ++){
      // d = sphere_dist(lon[i],lat[i],lon[j],lat[j]);
      d = haver_dist(lon[i],lat[i],lon[j],lat[j]);
      
      // if( (d == NA) ){
      //   Rcpp::Rcout << lon[i] << " " << lat[i] << " " << lon[j]<< " "  << lat[j]<< " "  << std::endl;
      // }
      out(j,i)=d;
      out(i,j)=d;
    }
...
NumericMatrix circ_dist_year(NumericVector x){
  int n = x.size();
  NumericMatrix out(n,n);
  
  for (int i = 0; i < n - 1; i++){
    double long1s = 2.0 * x[i] * M_PI/12.0;
    
    for (int j = i + 1; j < n ; j ++){
      
      double long2s = 2.0 * x[j] * M_PI/12.0;
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List list_dists(vec lon,vec lat,List neighs){
  int n = lon.size();
  List dists(n);
  ...
  for(int i=1 ; i < n ; i++){
    ...
    NumericMatrix lon_neigh(mat_size,1); 
    NumericMatrix lat_neigh(mat_size,1); 
    lon_neigh(0,0) = lon[i];
    lat_neigh(0,0) = lat[i];
    
    for(int j=1; j < mat_size; j++){
      lon_neigh(j,0) = lon[ind_neigh[j-1]]; 
      lat_neigh(j,0) = lat[ind_neigh[j-1]]; 
    }
    
    NumericMatrix dist_mat(mat_size,mat_size);
    dist_mat = dist_mat_sphere(lon_neigh,lat_neigh);
    dists[i] = dist_mat;
  }
  return dists;
}
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List list_circ_difs_year(vec hours,List neighs){
  int n = hours.size();
  List dists(n);
  ...
  for(int i=1 ; i < n ; i++){
    ...
    NumericMatrix dist_mat(mat_size,mat_size);
    dist_mat = circ_dist_year(times_neigh);
    dists[i] = dist_mat;
  }
  return dists;
}
...
// [[Rcpp::export]]
List list_grid_dists(vec lon,vec lat,vec lon_grid,vec lat_grid,List neighs){
  int n = lon_grid.size();
  List dists(n);
  for(int i=0 ; i < n ; i++){
    IntegerVector ind_neigh = neighs[i];
    ...
    NumericMatrix lon_neigh(mat_size,1); 
    NumericMatrix lat_neigh(mat_size,1); 
    lon_neigh(0,0) = lon_grid[i];
    lat_neigh(0,0) = lat_grid[i];
...
double example5(double theta,double vartheta,double time, 
                double ch, double ct, double tau, double eps){
  double temp = exp(- theta / ch);
  
  double res = pow((1 - eps) / (1 - eps * temp * cos(vartheta)),tau);
  return res * exp(-time / ct);
}

// [[Rcpp::export]]
double example6(double theta,double vartheta,double time,
                double ch, double ct, double alp){
  double temp1 = exp(- theta / ch);
  double temp2 = pow(0.5,alp) * (1 - temp1 * cos(vartheta));
  double res = (1.0 - temp2) * exp(-time / ct);
  return res;
}

// [[Rcpp::export]]
double example7(double theta,double vartheta,double time,
...
// [[Rcpp::export]]
double example8(double theta,double vartheta,double time,
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List cor_list(List dist_all, List circ_all, List times_all,vec pars,int mod_choice,int n){
  List cov(n); 
  double one_cov = 1;
  
  for(int i=0 ; i < n; i++){
    mat dists =as<arma::mat>(dist_all[i]);
    mat circ_times =as<arma::mat>(circ_all[i]);
    mat times =as<arma::mat>(times_all[i]);
    
    int n_mat = dists.n_rows;
    ...
    for(int j = 0; j < n_mat ; j++){
      for(int k = j ; k < n_mat ; k++){
        ...
        if(mod_choice == 5){ // Example 2
          one_cov = example5(dists(j,k),circ_times(j,k),times(j,k),
                             pars[0],pars[1],pars[2],pars[3]);
        }
        if(mod_choice == 6){ // Example 2
          one_cov = example6(dists(j,k),circ_times(j,k),times(j,k),
                             pars[0],pars[1],pars[2]);
        }
        if(mod_choice == 7){  // Simple Model
          one_cov =  example7(dists(j,k),circ_times(j,k),times(j,k),
                              pars[0],pars[1],pars[2],pars[3],pars[4]);
        }
        if(mod_choice == 8){  // Simple Model
          one_cov =  example8(dists(j,k),circ_times(j,k),times(j,k),
                              pars[0],pars[1],pars[2],pars[3],pars[4]);
        }
        if(mod_choice == 9){ // Example 2
          one_cov = example9(dists(j,k),circ_times(j,k),times(j,k),
                             pars[0],pars[1],pars[2],pars[3]);
        }
        
        if(mod_choice == 10){ // Example 2
          one_cov = example10(dists(j,k),circ_times(j,k),times(j,k),
                              pars[0],pars[1],pars[2],pars[3]);
        }
...
double w_update(int obs_ind,bool is_observed,vec &w,double tau2,vec y , 
                List neigh,IntegerVector U,List B, List F){ 
  ...
  if(is_observed){
    ...
    m_w = B_w.t() * w_N / F_w(0,0) + U_terms[0] + (y[obs_ind])/tau2;
  } else{
...
// [[Rcpp::export]]
vec w_update_all(vec w,IntegerVector is_obs, double tau2,vec y ,List neigh,
                 List U,List B, List F_scale){
  
  int n = w.size();
  vec out(n);
  
  out[0] = w_update0(0,is_obs[0], w, tau2, y, neigh,U[0], B,  F_scale);
  
  for(int j=1 ; j < n ; j++){
    out[j] = w_update(j,is_obs[j], w, tau2, y, neigh,U[j], B,  F_scale);
  } 
...
double tau2_update(double a, double b,vec w,vec y){ 
  
  int n = w.size();
  double a_s = a + ((double)n)/2.0;
  double b_s = b + sum(pow(y - w,2.0))/2.0;
...
List cov_update(vec covs, vec cand,List B,List F,double current_like,double sig2,
                ...
                List dists, List circ_difs, List times){
  int n = w.size();
  
  /// covs, output &B,List &F &current_like, count
  
  List cors_temp = cor_list(dists,circ_difs,times,cand,mod_choice,n );
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
NumericMatrix pred_NNGP(vec times,vec lon, vec lat,vec grid_times,vec grid_lon, vec grid_lat,
                        List grid_neighs, NumericVector sig2, NumericVector tau2,
                        NumericMatrix w,NumericMatrix cov_pars,int mod_choice){
  ...
  dists = list_grid_dists(lon,lat,grid_lon,grid_lat, grid_neighs);
  List time_difs = list_grid_time_difs(times,grid_times,grid_neighs);
  List circ_difs = list_grid_circ_difs(times,grid_times,grid_neighs);
  
  for(int i=0 ; i < rep; i++){
    vec rho = cov_pars(i,_);
    List cors = cor_list( dists, circ_difs, time_difs,rho,mod_choice,g );
    mat this_thing = cors[0];
    List Bs = Bmats_g( cors, g);
    List Fs = Fmats_g( cors, Bs, g); 
    vec w_sub = w(i,_);
    // Rcpp::Rcout << "here = " << i << std::endl;
    ...
    for(int j=0 ; j < g; j++){
      ...
      mat Ftemp = Fs[j];
      if(Ftemp(0,0) > 0 ){
        res(i,j) = rnorm(1,Bw[0],sqrt(sig2[i]*Ftemp(0,0) + tau2[i] ))[0];
      } else{
        res(i,j) = Bw[0];
        Rcpp::Rcout << "problem here" << std::endl;
      }
    }
    Rprintf("\r Prediction number: \v%d", i+1);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
NumericMatrix pred_NNGP2(vec times,vec lon, vec lat,vec grid_times,vec grid_lon, vec grid_lat,
                         List grid_neighs, NumericVector sig2, NumericVector tau2,
                         NumericMatrix w,NumericVector cov_pars,int mod_choice){
